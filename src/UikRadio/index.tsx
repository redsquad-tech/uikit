...
export default class UikRadio extends Component<Props, State> {
    ...
    renderSingleRadio = (option: Option, key: number) => {
        ...
        return (
            <div key={key} onClick={this.changeValue(option.value)} className={cn(s.input, disabled ? s.disabled : '')} >
                <input type="radio" disabled={disabled} onChange={this.changeValue(option.value)} checked={currentValue === option.value}/>
                {option.label}
            </div>
        )
    }
